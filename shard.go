...
func exec(db *sql.DB, handler DBHandler, tp TablePartition) {
	_ = handler(db, tp)
}

func (s *Shard) Write(handler DBHandler) {
	s.Executor(s.Master.GetDB(), handler, s.tp)
}
